...
const WorkoutFeed = (props) => {
  ...
  const handleClickOpen = () => {
    setOpen(true);
    setWorkoutName(workoutInfo.workoutName);
    setWorkoutDescription(workoutInfo.workoutDescription);
  };
  ...
  const handleSubmit = (event) => {
    ...
    fetch(`http://localhost:8082/api/v1/user/workout/${workoutInfo.id}`, requestOptions)
      .then(async (response) => {
  ...
  const addComment = async () => {
    const response = await fetch(`http://localhost:8082/api/v1/user/workouts/${workoutInfo.id}/comments`, {
      method: "POST",
      headers: {
  ...
  const deleteWorkout = async (workoutId) => {
    const response = await fetch(`http://localhost:8082/api/v1/user/workouts/${workoutId}`, {
      method: "DELETE",
      headers: {
        "Content-Type": "application/json"
      }
  ...
  const deleteComment = async (commentId) => {
    const response = await fetch(`http://localhost:8082/api/v1/user/workouts/comment/${commentId}`, {
      method: "DELETE",
      headers: {
        "Content-Type": "application/json"
      }
  ...
  return (
    <Card sx={{ marginTop: "1em" }}>
          ...
          <div>
            ...
            >
              <MenuItem onClick={handleClose}>
                <IconButton onClick={() => handleDeleteWorkout(workoutInfo.id)}>
      ...
      <Typography variant="h6" color="textPrimary" style={{ fontWeight: 'bold' }}>
        {props.data.workoutName}
      </Typography>
      <Typography variant="body1" color="textSecondary">
        {props.data.workoutDescription}
      </Typography>
      </CardContent>
      <CardMedia
        component="img"
